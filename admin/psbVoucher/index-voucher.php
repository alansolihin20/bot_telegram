...
$sql_migrasi = "SELECT * FROM psb_voucher  WHERE MONTH(waktu_input) = '$bulan_filter' 
...
<div class="container mt-4">
    <h3>Data PSB Voucher</h3>
    <a href="hapus-voucher.php?bulan=<?= $bulan_filter ?>&tahun=<?= $tahun_filter ?>" 
        ...
        <tr>
          ...
          <td>
            <?php if (empty($row['port']) && empty($row['index_rx'])): ?>
              <a href="respon_voucher.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">Respon</a>
            <?php else: ?>
